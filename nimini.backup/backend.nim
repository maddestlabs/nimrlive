...
method generateProcDecl*(backend: CodegenBackend; name: string; params: seq[(string, string)]; returnType: string = ""; indent: string = ""): string {.base.} =
  ## Generate code for a procedure/function declaration
  ## returnType is an empty string if no return type is specified
